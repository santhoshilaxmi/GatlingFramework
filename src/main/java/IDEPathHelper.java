...
public class IDEPathHelper {
  ...
  static {
    try {
      ...
      Path mavenSrcMainDirectory = projectRootDir.resolve("src").resolve("main");

      mavenSourcesDirectory = mavenSrcMainDirectory.resolve("java");
      mavenResourcesDirectory = mavenSrcMainDirectory.resolve("resources");
      mavenBinariesDirectory = mavenTargetDirectory.resolve("test-classes");
      resultsDirectory = projectRootDir.resolve("gatling");
